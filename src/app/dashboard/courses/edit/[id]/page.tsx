...
import { UploadCloud, X, Loader2, ChevronRight, Info } from "lucide-react";
...
const courseSchema = z.object({
    ...
    category_id: z.coerce.string().min(1, "Kategori zorunludur"),
...
export default function EditCoursePage() {
    ...
    const [editCourse, { isLoading: isUpdatingCourse }] = useEditCourseMutation();
    const { data: categories, isError: isCategoryError, isSuccess: isCategorySuccess } = useGetCourseCategoriesQuery({ page: 1, token }, { skip: !token });

    const { data: courseDetailData, isLoading: isDetailLoading, error: detailError, isSuccess: isDetailSuccess } = useGetCourseDetailQuery(courseId, {
    ...
    const { register, handleSubmit, control, watch, setValue, reset, formState: { errors } } = useForm<CourseFormValues>({
    ...
    useEffect(() => {
        if (courseDetailData && categories?.results) {

            // 2. Format ISO Dates safely to HTML Input format
            const formattedStartDate = courseDetailData.start_date ? courseDetailData.start_date.split('T')[0] : "";
            const formattedEndDate = courseDetailData.end_date ? courseDetailData.end_date.split('T')[0] : "";

            // Format HH:mm using slice safely avoiding full timezone tails
            const formattedStartTime = courseDetailData.start_time ? courseDetailData.start_time.slice(0, 5) : "";
            const formattedEndTime = courseDetailData.end_time ? courseDetailData.end_time.slice(0, 5) : "";
            ...
            reset({
                ...
                price: Number(courseDetailData.price) || 0,
                discounted_price: courseDetailData.discounted_price ? Number(courseDetailData.discounted_price) : undefined,
                category_id: courseDetailData.category ? String(courseDetailData.category) : "",
                capacity: courseDetailData.quota || 0, // Explicitly pull correct key
            ...
            if (courseDetailData.image_url) {
                setUploadedImage({ url: courseDetailData.image_url });
            }
        }
    }, [courseDetailData, categories, reset]);
    ...
    const onSubmit = async (data: CourseFormValues) => {
        ...
        const updateCoursePayload: any = {
            ...
            is_private_lesson: Boolean(
                data.is_private_lesson === true ||
                (data.is_private_lesson as unknown as string) === "true"
            ),
        };

        // Only send new image if it was uploaded in this session
        if (uploadedImage.public_id) {
            updateCoursePayload.image_url = uploadedImage.url;
            updateCoursePayload.image_public_id = uploadedImage.public_id;
        } else if (uploadedImage.url) {
            updateCoursePayload.image_url = uploadedImage.url; // Keep URL stable if backend requires
        }

        try {
            const response = await editCourse({ id: courseId, body: updateCoursePayload }).unwrap();
            const successMessage = response?.message || "Kurs başarıyla güncellendi";
            toast.success(successMessage);
            router.push("/dashboard/courses");
        } catch (error: any) {
            console.error("Edit course failed:", error);
            const backendMessages = getErrorMessages(error?.data);
            toast.error(backendMessages[0] || error?.data?.message || "Kurs güncellenirken bir hata oluştu.");
        }
    };

    if (isAuthLoading || (!isAuthorized && typeof window !== "undefined") || isDetailLoading) {
        return (
    ...
    if (detailError || (!courseDetailData && !isDetailLoading)) {
        return (
                ...
                <div className="p-4 sm:p-8 max-w-[1200px] mx-auto space-y-6">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center text-sm text-slate-500">
                            ...
                            <span className="hover:text-slate-900 cursor-pointer" onClick={() => router.push(`/dashboard/courses/detail/${courseId}`)}>Kurs Detayı</span>
                            <span className="mx-2">›</span>
                            <span className="font-medium text-slate-900">Düzenle</span>
                        </div>

                        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                            <div className="p-6 border-b border-slate-100">
                                <h1 className="text-2xl font-bold text-slate-900">Kursu Düzenle: {courseDetailData?.name}</h1>
                                <p className="text-slate-500 mt-1">
                                    Kurs bilgilerini güncellemek için aşağıdaki alanları düzenleyin.
                                </p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-8">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Controller
                                        ...
                                        name="level"
                                        render={({ field }) => (
                                            <Select onValueChange={field.onChange} value={['A1', 'A2', 'B1', 'B2', 'C1', 'C2'].find(opt => String(opt) === String(field.value)) || undefined}>
                                    ...
                                    <div className="flex flex-col gap-6">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Controller
                                                ...
                                                name="category_id"
                                                render={({ field }) => (
                                                    <Select onValueChange={field.onChange} value={formattedCategoryOptions.find((opt: { value: string }) => String(opt.value) === String(field.value))?.value || undefined}>
                                ...
                                <div className="space-y-2">
                                    <Label className="text-sm font-semibold text-slate-700">Yeni Kurs Görseli Yükle</Label>
                                        ...
                                        {isUploadingImage ? (
                                            ...
                                            <div className="flex flex-col items-center gap-2 text-center px-4">
                                                ...
                                                <span className="text-xs text-slate-500">
                                                    Yeni görsel seçerseniz eskisi silinir
                                                </span>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <Label className="text-sm font-semibold text-slate-700 mb-2 block">Yüklenen Görseller (Geçerli Önizleme)</Label>
                                        {uploadedImage && (
